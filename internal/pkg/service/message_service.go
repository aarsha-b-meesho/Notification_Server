...
// 		log.Printf("CreateSMS: Error inserting SMS into database: %v", err)
// 		return fmt.Errorf("error inserting SMS into database: %w", err)
// 	}
// 	topic := "SMS"
...
// 	if err != nil {
// 		log.Printf("CreateSMS: Error producing message to Kafka: %v", err)
// 		return fmt.Errorf("error producing message to Kafka: %w", err)
// 	}

// 	log.Printf("CreateSMS: Successfully created SMS with ID %s and sent to Kafka", sms.ID)
...
// 	if err != nil {
// 		log.Fatalf("ProcessMessages: Failed to subscribe to Kafka topic: %v", err)
// 	}
// 	for {
// 		msg, err := s.kafkaConsumer.ReadMessage(-1)
// 		if err != nil {
// 			log.Printf("StartConsumingMessages: Failed to read message from Kafka: %v", err)
// 			continue
// 		}
// 		s.queueMu.Lock()
// 		if s.queueSwitch {
...
// 	if len(messages) == 0 {
// 		log.Println("process_messages: No messages to process")
...
// 		{"data": map[string]string{"comments": "No messages to process"}},
// 	}
// }
...
// 	if err != nil {
// 		log.Printf("check_sms_existence: Failed to check ID existence for %s: %v", msgID, err)
...
// 	if err != nil {
// 		log.Printf("retrieve_sms_details: Failed to retrieve SMS details for %s: %v", msgID, err)
// 		return sms, fmt.Errorf("failed to retrieve SMS details: %w", err)
// 	}
// 	return sms, nil
// }
...
// 	if err != nil {
// 		log.Printf("check_blacklist_status: Error checking blacklist status for %s: %v", phoneNumber, err)
...
// 	if err != nil {
// 		log.Printf("handle_blacklisted_sms: Failed to update SMS status for %s: %v", sms.ID, err)
// 		return nil, fmt.Errorf("failed to update SMS details: %w", err)
// 	}

// 	err = s.index_sms_in_elasticsearch(sms)
// 	if err != nil {
// 		log.Printf("handle_blacklisted_sms: Failed to index SMS in Elasticsearch for %s: %v", sms.ID, err)
// 		return nil, fmt.Errorf("failed to index SMS in Elasticsearch: %w", err)
// 	}
...
// 	err := s.db.UpdateSMSStatus(sms.ID, sms.Status, sms.FailureComments)
// 	if err != nil {
// 		log.Printf("handle_successful_sms: Failed to update SMS status for %s: %v", sms.ID, err)
// 		return nil, fmt.Errorf("failed to update SMS details: %w", err)
// 	}

// 	err = s.index_sms_in_elasticsearch(sms)
// 	if err != nil {
// 		log.Printf("handle_successful_sms: Failed to index SMS in Elasticsearch for %s: %v", sms.ID, err)
// 		return nil, fmt.Errorf("failed to index SMS in Elasticsearch: %w", err)
// 	}

// 	log.Printf("handle_successful_sms: SMS ID %s processed successfully", sms.ID)
// 	return map[string]interface{}{
// 		"data": map[string]string{"comments": "No failure"},
// 	}, nil
// }
